...
<<<<<<< HEAD
import { fetchAssignments as fetchAssignmentsApi, createAssignment as createAssignmentApi } from '../api/api';
import { getAssignments, markAssignmentCompleted, getSubmissions, submitAssignmentWork } from '../data/assignments';
=======
...
<<<<<<< HEAD
        ...
        const loadAssignments = async () => {
            setIsLoading(true);
            try {
                const data = await fetchAssignmentsApi(currentUser?.role || 'student');
                setAssignments(data);
            } catch (err) {
    ...
    const handleCreateAssignment = async (e) => {
        ...
        try {
            await createAssignmentApi(newAssignment);
            
            // Refresh local list
            const data = await fetchAssignmentsApi(user?.role || 'instructor');
            setAssignments(data);
            
            setIsAdding(false);
            setNewAssignment({ title: '', course: 'General', dueDate: new Date().toISOString().split('T')[0] });
            alert('Deliverable deployed successfully to the workspace!');
        } catch (err) {
            console.error("Failed to deploy assignment:", err);
            alert('Technical Error: Failed to broadcast assignment details.');
        } finally {
...
<<<<<<< HEAD
    const handleCompleteAssignment = async (id) => {
        ...
        // Refresh from backend to keep view updated
        try {
            const data = await fetchAssignmentsApi(user?.role || 'student');
            setAssignments(data);
        } catch (e) {
            console.error(e);
        }
...
<<<<<<< HEAD
                                {isAdding ? 'Cancel Action' : 'Add Assignment'}
=======
...
<<<<<<< HEAD
                                <button type="submit" disabled={isPublishing} className="btn-primary w-full py-5 rounded-2xl text-lg font-black uppercase tracking-widest shadow-2xl shadow-brand-primary/30 active:scale-[0.98] transition-transform disabled:opacity-70 disabled:cursor-not-allowed">
                                   {isPublishing ? (
